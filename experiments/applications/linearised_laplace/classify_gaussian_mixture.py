...
if __name__ == "__main__":
    ...
    def loss_p(v, x, y):
        return jnp.mean(
            jax.vmap(bnn_util.loss_single)(model.apply(unflatten(v), x), y), axis=0
        )
    ...
    for epoch in range(n_epochs):
        ...
        if epoch % 10 == 0:
            acc = bnn_util.accuracy(
                model.apply(unflatten(variables), x_train[idx]), y_train[idx]
            )
    ...
    calib_loss = bnn_util.calib_loss(model.apply, unflatten)
