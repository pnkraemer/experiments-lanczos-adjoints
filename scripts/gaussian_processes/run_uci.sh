...
num_matvecs=1  # no point setting below 10, gpytorch overrides anyway
cg_tol=1e1  # split the middle between GPyTorch and papers
rank_precon=1  # like in GPyTorch
num_epochs=1  # like in most papers
num_samples=11  # less than anywhere

for seed in 1 2 3
do
  for dataset in  concrete # elevators # concrete # # #power_plant elevators protein kin40k kegg_directed kegg_undirected
  do
    python experiments/applications/gaussian_process/train/optim_logml_gpytorch_adaptive.py \
      ...
      --dataset $dataset --rank_precon $rank_precon;
