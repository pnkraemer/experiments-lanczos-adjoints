...
def test_adjoint_matches_jax_dot_vjp(nrows, krylov_depth, reortho):
...
@pytest_cases.parametrize("nrows", [1])
@pytest_cases.parametrize("krylov_depth", [1, 5, 10])
@pytest_cases.parametrize("reortho", ["none", "full", "full_with_sigma"])
def test_adjoint_matches_jax_dot_vjp_for_complex_types(nrows, krylov_depth, reortho):
    # todo: see which components simplify for symmetric matrices
