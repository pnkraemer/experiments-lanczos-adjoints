...
def test_slq_spd_value_and_grad_matches_autodiff(n=10):
    ...
    # Implementation
    integrand = deprecated.integrand_spd_value_and_grad(np.log, order, matvec)
    estimate = hutchinson.hutchinson(integrand, sampler)
...
def test_slq_spd_custom_vjp_recursive_matches_slq_spd_custom_vjp(n=10):
    ...
    reference2 = lanczos_extensions.integrand_spd_custom_vjp(np.log, order, matvec)
    reference2 = hutchinson.hutchinson(reference2, sampler)
    implementation = deprecated.integrand_spd_custom_vjp_recursive(
        np.log, order, matvec
    )
